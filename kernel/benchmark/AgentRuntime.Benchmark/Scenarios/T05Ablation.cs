...
public sealed class T05Ablation : IBenchmarkScenario
{
    ...
    public async Task RunAsync(BenchmarkContext context, CancellationToken cancellationToken)
    {
        ...
        var s1 = await Runner.ViaEngineAsync(session, Corpus.Salt(context.RunNonce, "T05/session") + "我叫张总，请记住我的称呼。", cancellationToken);
        ...
        var b1 = await Runner.ViaEngineAsync(both, Corpus.Salt(context.RunNonce, "T05/rules+session") + "我叫张总，请记住我的称呼。", cancellationToken);
