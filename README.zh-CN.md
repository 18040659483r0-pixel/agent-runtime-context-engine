...
# Agent Runtime / Context Engine 架构（设计说明 · 第四版）

> 日期：2026-09-17　作者：本人
> 版本：**v4.0**（2026-09-17，**架构收敛**：动态区准入 · 工具面与审批闸门（抽象）· 观测台 · 能力分层与语料闸门 · **MIT 许可**）　上一版 v3.0 见 [tag v3.0](https://github.com/18040659483r0-pixel/agent-runtime-context-engine/tree/v3.0)；更早的 v2.0 / v1.0 同名保留。
...
## ⑥ v4.0 变更（架构收敛 · 2026-09-17）

本版**不新增主张**，只把**已实现**的部分收敛进架构描述，并把几处口径写清楚。

### 6.1 动态区的确定性准入（Current Tail / Dynamic Draft / Semantic Focus）

稳定前缀之外，允许**三个受控动态区**存在，但进入规则是**确定性**的（不是「模型想写就写」）：

| 区 | 作用 | 准入规则（确定性） |
|---|---|---|
| **Current Tail**（白板） | 承接「本任务最新状态 / 待办」，取代「重写摘要」 | 有上限；写入口唯一；**只追加不改前部** |
| **Dynamic Draft**（草稿） | 存放**待裁决**的内容（未晋级的知识 / 承诺） | 显式声明才进；未晋级**不得**混入稳定区 |
| **Semantic Focus**（焦点） | 把注意力钉到与本轮相关的事件 / 段落 | 由事件标签**推导**，可复算、可解释；悬空标签**告警** |

**共同约束**：动态区**永不前置**（§4.4 / §4.12 反例）；它们只影响**注意力**，不改**物理历史**（§①）。

### 6.2 工具面与审批闸门（抽象描述）

> 本节**刻意抽象** —— 机制细节不属于本文范围。

- **能力面**：内核自带最小工具集（文件读 / 写 / 改、目录列举、命令执行）；**同名同语义**，**每个工具可单独开关**。
- **人在环**：凡**改本机状态**或**不可逆**的动作，一律**先问人、后执行**；**模型不得自批**；默认**拒绝**（fail-closed）—— 不点头就不做。
- **可见性**：授权面给出**将要执行的真实动作**（逐字）；**一次一批、逐项确认**；授权层与执行层分离。
- **非沙箱（诚实声明）**：工具面**不构成安全沙箱** —— 它拦不住越界行为，安全性由**审批质量**承担 ⇒ 不把它写成安全边界。
- **可观测**：每次工具调用与结果进事件流，可复算；观测面**只读**。

### 6.3 观测台与不变量（同一壳）

运行时附一个**观测台**：左对话、右**区栈逐字节账本**（各段字节 / 指纹 / 版本 / 次序 + 本轮命中与开销）。价值在**可证伪**：

- **只读**：面板只渲染，**不改变**送进模型的一个字节；
- **不是注入源**：写动作只能触发既有写入口，不产生新段；
- **可复算**：面板输出为纯文本，可落盘、可 diff、可进实验记录。

### 6.4 能力分层与语料闸门

- **能力分三层**：常驻（L1/L2 抽象）→ 按需（L3 明细）。**常驻只留抽象与索引**，明细**落盘 + 按需装载** —— 与 §4.4 同一动机（不让常驻区膨胀蚕食前缀缓存收益）。
- **语料闸门**：对外的**实验语料**是真实资料的**去隐私切片**（约 10k token 档，按字符预算切片 + token 校准）；**完整语料不对外**（发布前由扫描器把关：私有语料 / 本机路径 / 内网地址 / 凭据 / 网盘一律硬红线）。

### 6.5 工程状态（抽象）

预演练按**四阶段**推进：**影子模式**（同轮双跑比对组装出的上下文）→ **双跑对照**（同一批任务两侧各跑一遍；判据：**正确率不降、规模与命中可解释**）→ **有限切换**（低风险任务类型先切，逐项勾掉）→ **全切**（保留回退通道 + 回退演练）。**前三阶段已有可复算记录；全切尚未进行。**

> **口径纪律**：两侧的**平台固定开销分开记账**，只比**共享段**；**不同单位的数不并列比较**（「项 / 批 / 次」必须带单位，旧数据**不反推**）。

### 6.6 许可

- **内核与观测台（TUI）采用 MIT 许可**（见仓库根 `LICENSE`）：可自由使用、修改、分发（保留版权与许可声明）。
- **不在开源范围**：内部知识库、私有语料与内部工程文档（它们不属于本仓库的发布物）。

### 6.7 变更记录

| 版本 | 日期 | 变更 |
|---|---|---|
| v1.0 | 2026-09-11 | 初版（前缀缓存最优的分层智能体架构） |
| v2.0 | 2026-09-14 | Append-only 会话流 + 收尾 / 水位线 + 稳定前缀边界 |
| v3.0 | 2026-09-14 | 加入**首轮实测**（64-token 块；节省 0% → 78.5%；反例 4.7×） |
| **v4.0** | **2026-09-17** | **架构收敛**：动态区三件（§6.1）· 工具面与审批闸门（§6.2，抽象）· 观测台（§6.3）· 能力分层与语料闸门（§6.4）· 工程状态（§6.5）· **MIT 许可**（§6.6） |
